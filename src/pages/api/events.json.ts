...
interface Event {
  ...
  isCurated?: boolean;
  country?: 'CA' | 'US'; // Track country
  eventType?: 'webinar' | 'trade-mission' | 'conference' | 'workshop' | 'networking';
}
...
interface EventOrganizer {
  ...
  eventsUrl?: string;
  description: string;
  scrapingStrategy?: 'jsonld' | 'html-links' | 'curated';
  country: 'CA' | 'US';
}

// EXPANDED CANADIAN ORGANIZATIONS
const canadianOrganizers: EventOrganizer[] = [
...
// US ORGANIZATIONS
const usOrganizers: EventOrganizer[] = [
...
// EXPANDED CURATED EVENTS - Canadian + US
...
async function trackEventClick(eventId: string, organizerId: string, country: string) {
  // This would integrate with your analytics platform
  // Examples: Google Analytics, Plausible, Mixpanel, etc.
  console.log(`📊 Analytics: Event clicked - ${eventId} (${organizerId}, ${country})`);
  
  // Example integration with Google Analytics (if you have it):
...
function extractFromJSONLD(html: string, organizer: EventOrganizer): Event[] {
  const events: Event[] = [];
...
export async function GET({ request }: { request: Request }) {
  ...
  try {
    ...
    const allEventsPromises = eventOrganizers.map(org => 
      fetchEventsFromOrganizer(org).catch(() => [])
    );
    ...
    console.log(`✅ ${filteredEvents.length} total (CA: ${canadianEvents}, US: ${usEvents}) (${(fetchTime/1000).toFixed(1)}s)\n`);
